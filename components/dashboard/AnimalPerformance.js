...
export default function AnimalPerformance() {
  ...
  const calculatePerformance = () => {
    return mockAnimals.map(animal => {
      const roi = animal.valorVenda 
        ? ((animal.valorVenda - animal.custoTotal) / animal.custoTotal * 100)
        : 0
      
      const profit = animal.valorVenda ? (animal.valorVenda - animal.custoTotal) : 0
      
      const performance = roi > 30 ? 'Excelente' : 
                         roi > 15 ? 'Boa' : 
                         roi > 0 ? 'Regular' : 
                         animal.valorVenda ? 'Ruim' : 'Em Andamento'
      
      const riskLevel = animal.custoTotal > 5000 ? 'Alto' :
                       animal.custoTotal > 2000 ? 'Médio' : 'Baixo'
  ...
  const filters = [
    ...
    { id: 'sold', label: 'Vendidos', count: animals.filter(a => a.valorVenda).length },
    { id: 'active', label: 'Ativos', count: animals.filter(a => a.situacao === 'Ativo').length },
    { id: 'profitable', label: 'Lucrativos', count: animals.filter(a => a.roi > 0).length },
    { id: 'high-risk', label: 'Alto Risco', count: animals.filter(a => a.riskLevel === 'Alto').length }
  ]
