...
export default function SmartAlerts() {
  ...
  const generateSmartAlerts = () => {
    ...
    mockAnimals.forEach(animal => {
      ...
      if (animal.custoTotal > 5000) {
        newAlerts.push({
          ...
          message: `${animal.serie} ${animal.rg} tem custo de R$ ${animal.custoTotal.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}`,
          animal: animal,
    ...
    const totalInvested = mockAnimals.reduce((acc, a) => acc + a.custoTotal, 0)
    const avgCost = totalInvested / mockAnimals.length
